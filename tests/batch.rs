...
#[test]
fn sixty_five_due_archives_cross_batch_boundaries_without_losing_counts() {
    let mut fixture = Fixture::new();
    let ids = add_archives(&fixture, 65);
    ...
    assert_eq!(report.examined, 65);
    assert_eq!(report.deleted, 65);
    assert_eq!(report.skipped, 0);
    assert_eq!(report.logical_bytes_removed, expected_bytes);
    assert_eq!(report.entries.len(), 65);
...
fn oversized_batch_journal_is_rejected_before_any_recovery_mutation() {
    let mut fixture = Fixture::new();
    let ids = add_archives(&fixture, 33);
    let items = batch_items(&fixture, &ids);
...
fn pinned_and_excluded_live_writers_are_filtered_before_forming_batches() {
    let mut fixture = Fixture::new();
    let ids = add_archives(&fixture, 34);
    let pinned = &ids[0];
    let excluded = &ids[1];
    fixture
        .c
