...
def read_case(path):
    ...
    if (path.parent != root / "cases" or not re.fullmatch(r"[a-z0-9-]+\.json", path.name)
            ...
            or Path(case["fixture_root"]) != root / "fixtures" / path.stem):
...
def command(case):
    ...
    result += ["run", "--scheduled"] if case["scenario"] == "summary" else [
        "run" if case["scenario"].endswith("busy") else "preview"]
    return result
...
def read_manifest(root):
    ...
    for record in manifest["records"]:
        ...
        if (fixture.no_symlinks(root / record["db_path"]) != logical
...
def expected(case, manifest):
    ...
    removed = archived - set(case["busy_ids"]) if scenario == "one-busy" else set()
    eligible = len(removed) if scenario.endswith("busy") else (
...
def populate(case):
    ...
    count = 100000 if case["scenario"] == "summary" else (
        1000 if case["scenario"].startswith("mixed-") else 128)
    ...
    with closing(sqlite3.connect(database)) as connection:
        due = case["scenario"].endswith("busy") or case["scenario"].startswith("headers-")
...
def restore(case_path):
    ...
    if not case["scenario"].endswith("busy"):
        raise ValueError("only destructive contention fixtures need reset")
    root = fixture.owned_root(case["fixture_root"])
...
def measure(case_path, hyperfine, runs, rss, prepare_only):
    ...
    with held_locks(case):
        ...
        if contention:
            ...
            invocation += ["--ignore-failure", "--prepare", shlex.join(internal + ["_prepare", "--case", str(case_path)]),
        ...
        if rss:
            receipt["rss"] = sample_rss(case_path)
        if digest(case["binary"]) != expected_binary_hash:
...
def setup(args):
    ...
    for index, scenario in enumerate(scenarios):
        ...
        for tool, binary in order:
            ...
            case = {"benchmark_root": str(root), "fixture_root": str(root / "fixtures" / path.stem),
    ...
    fixture.write_json(root / "environment.json", {
        ...
        "cache": "warm filesystem cache; read-only fixtures reused, contention data restored outside timing",
...
def main():
    ...
    run.add_argument("--cases", nargs="+", choices=["mixed", "summary", "contention", "headers"],
    ...
    for action in ("_prepare", "_validate"):
